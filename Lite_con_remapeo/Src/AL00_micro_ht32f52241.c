...
//				 ADC
//-------------------------------------------
void (*func_ptr_callback_adc)(void) = empty_func_to_avoid_hard_fault;																 
...
static void micro_pwm_config(void)
{
	...
	*/
	...
	AFIO_GPxConfig(GPIO_PC, AFIO_PIN_3 , AFIO_FUN_MCTM_GPTM);		//BREAK PIN

	AFIO_GPxConfig(GPIO_PC, AFIO_PIN_2, AFIO_FUN_MCTM_GPTM);		//MT_CH0N - HIN3
	AFIO_GPxConfig(GPIO_PB, AFIO_PIN_1, AFIO_FUN_MCTM_GPTM);		//MT_CH1N - HIN1
	AFIO_GPxConfig(GPIO_PB, AFIO_PIN_3 , AFIO_FUN_MCTM_GPTM);		//MT_CH2N - HIN2
	...
	//MT_CH3 - Time before Ton
	TM_OutputInitStructure.Channel = TM_CH_3;
	TM_OutputInitStructure.Compare = 50;
	TM_OutputInit(HT_MCTM0, &TM_OutputInitStructure);


	TM_ClearFlag(HT_MCTM0,TM_FLAG_CH3CC);				//IRQ de time_before_ton
	...
	TM_IntConfig(HT_MCTM0,TM_INT_CH3CC, DISABLE);
...
{	
	if(TM_GetFlagStatus(HT_MCTM0,TM_FLAG_BRK0))
	{
		...
		TM_ClearFlag(HT_MCTM0,TM_FLAG_CH3CC);

		__hardware_gpio_output_toggle(GPIOB, 11);					//GPIO aux para monitoreo en OSC
	}
	
	if(TM_GetFlagStatus(HT_MCTM0,TM_FLAG_CH3CC))
	{
...
static void micro_adc_config(void)
{
	...
	AFIO_GPxConfig(GPIO_PB, AFIO_PIN_8, AFIO_FUN_ADC);			//Shunt AVG measure
	AFIO_GPxConfig(GPIO_PC, AFIO_PIN_5, AFIO_FUN_ADC);			//Vbus measure
	AFIO_GPxConfig(GPIO_PB, AFIO_PIN_7, AFIO_FUN_ADC);			//NTC potencia
	//AFIO_GPxConfig(GPIO_PA, AFIO_PIN_2, AFIO_FUN_ADC);			//NTC heater


	/* Continuous Mode, Length 1, SubLength 1                                                                 */
	ADC_RegularGroupConfig(HT_ADC, CONTINUOUS_MODE, 3, 1);
	...
	ADC_RegularChannelConfig(HT_ADC, ADC_CH_0, 0);	//SHUNT_AVG
	ADC_RegularChannelConfig(HT_ADC, ADC_CH_1, 1);	//VBUS
	ADC_RegularChannelConfig(HT_ADC, ADC_CH_3, 2);	//NTC_PWR
	//ADC_RegularChannelConfig(HT_ADC, ADC_CH_4, 3);	//NTC_HEATER
