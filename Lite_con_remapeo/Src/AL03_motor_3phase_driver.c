...
#define ZCD_FRACTION_ADVANCE_NORMAL							0.40		//Porcentaje de zcd_to_zcd de avance para desde el arranque
...
#define bemf_sense_zcd_sampled_end_toff_enable()							{TM_ClearFlag(HT_MCTM0,TM_FLAG_CH3CC); TM_IntConfig(HT_MCTM0,TM_INT_CH3CC,ENABLE);}
#define bemf_sense_zcd_sampled_end_toff_disable()							{TM_ClearFlag(HT_MCTM0,TM_FLAG_CH3CC); TM_IntConfig(HT_MCTM0,TM_INT_CH3CC,DISABLE);}
/* 
...
int32_t motor_3phase_init(void)
{
	...
	board_comp_bemf_rising_link_callback(exti3_callback,BOARD_COMP_BEMF_U_PHASE_SELECT);
	board_comp_bemf_rising_link_callback(exti4_callback,BOARD_COMP_BEMF_V_PHASE_SELECT);
	board_comp_bemf_rising_link_callback(exti5_callback,BOARD_COMP_BEMF_W_PHASE_SELECT);
...
/*******************************************************************************
*	Funcion que modifica el duty SP al punto pasado como parametro
*	
 *******************************************************************************/
void motor_3phase_speed_force (int32_t set_point)
{
	gv.pwm_duty_set_point = set_point;
}
...
void motor_3phase_phase_lead_change (int32_t modif)
{
	...
	const float lead_table[TABLE_MAX] = {ZCD_FRACTION_ADVANCE_NORMAL, 0.35, 0.30, 0.25};
