...
#define PWM_OPERATING_DUTY									50

/*Configuracion de PWM que se va a usar en regimen permanente */
#define PWM_OPERATING_DUTY_PER								50

/*Definicion de la velocidad de funcionamiento en regimen permanente */
#define OPERATING_SPEED										MOTOR_MID_SPEED_SEQ_PERIOD
...
#define SPEED_TOLERANCE_PERC							3		//Porcentaje aproximado de tolerancia de velocidad

#define TIME_TO_GET_RUNNING_TIMEOUT_mS					200

#define	ZCD_COUNT_MAX									12		//Cantidad de ZCD que se dan antes de actualizar duty y periodo
...
int32_t motor_3phase_init(void)
{
	...
	*/
	...
	gv.motor_electrical_period_us_sp = OPERATING_SPEED;
...
void motor_3phase_starting_state_machine(void)
{
	...
	static int32_t alignment_count, speed_status;
		...
		case STARTING_STATE_STEPS_CORRECT_TIMMING:
									...
									switch(gv.starting_sub_state)
									{
										...
										case STARTING_SUB_STATE_RUNNING:
											...
											if(zcd_count == ZCD_COUNT_MAX)
											{
												//Control de velocidad
												//gv.motor_electrical_period_us_avg = electrical_period_update_average((gv.time_from_zcd_to_zcd_avg<<1) + gv.time_from_zcd_to_zcd_avg);
												speed_status = speed_in_range(SPEED_TOLERANCE_PERC);
												if ( speed_status == SPEED_OVER)
												{
													//La velocidad esta por encima del SP y debo bajar el duty
													if ( (gv.pwm_duty_actual-SET_POINT_PWM_DUTY_INC_DEC) > SET_POINT_MIN_PWM_DUTY )
													{
														gv.pwm_duty_actual -= SET_POINT_PWM_DUTY_INC_DEC;
													}
													else
													{
														//Ya estoy en el duty minimo
													}
													
												}
												else if ( speed_status == SPEED_LOWER )
												{
													//La velocidad esta por debajo del SP y debo subir el duty
													if ( (gv.pwm_duty_actual+SET_POINT_PWM_DUTY_INC_DEC) < SET_POINT_MAX_PWM_DUTY )
													{
														gv.pwm_duty_actual += SET_POINT_PWM_DUTY_INC_DEC;
													}
													else
													{
														//Ya estoy en el duty maximo
													}
												}
												else
												{
													//La velocidad esta en rango
												}
...
void zcd_event_freewheel_period_measure(void)
{
	...
	else if(gv.starting_state == STARTING_STATE_FREEWHEEL_PERIOD_MEASURE)
	{
		...
		//load_motor_electrical_period_vector((gv.time_from_zcd_to_zcd_avg<<1) + gv.time_from_zcd_to_zcd_avg);
...
void calculate_times (void)
{
	...
	common_update_average(gv.motor_electrical_period_us_avg, (gv.time_from_zcd_to_zcd_avg<<1) + gv.time_from_zcd_to_zcd_avg, AVG_FACTOR_ELECTRICAL_PERIOD);
	...
	gv.time_var_comm = gv.motor_comm_seq_period_us + (int32_t)(gv.time_t_error);
...
void load_motor_electrical_period_vector (int32_t value)
{
	for (int32_t i = 0; i < (MAX_VECTOR_LENGTH-1); i++)
	{
		gv.motor_electrical_period_vector[i] = value;
	}
}



int32_t electrical_period_update_average (int32_t value)
{
	static int32_t index = 0;

	gv.motor_electrical_period_vector[index] = value;
	
	index++;
	index = index % MAX_VECTOR_LENGTH;

	int32_t average = 0;

	for (int32_t i = 0; i < (MAX_VECTOR_LENGTH-1); i++)
	{
			average += gv.motor_electrical_period_vector[i];
	}

	return (average/MAX_VECTOR_LENGTH);
}
...
*	La idea de esta funcion es que haga una conmutacion al momento de detectar
...
{
	//ZCD en fase U
	//__hardware_gpio_output_set(GPIOB, 11);
	zcd_event();
...
/*******************************************************************************
*	Funcione de chequeo de velocidad - Jose
*	
*	Esta funcion controla si la velocidad del motor esta dentro del rango pasado
*	como parametro. Entrega una aproximacion para hacer mas eficiente el calculo
*	range - Rango en % (PE 10 = 10%)
*	devuelve SPEED_OVER, SPEED_LOWER o SPEED_OK
 *******************************************************************************/
int32_t speed_in_range (int32_t range)
{
	int32_t periodo_avg = gv.motor_electrical_period_us_avg;
	int32_t periodo_var;

	//El *10 es para llevarlo de porciento a por miles y poder despues dividirlo por 1024
	periodo_var = (((gv.motor_electrical_period_us_sp * range) * 10) >> 10);

	if( periodo_avg > (gv.motor_electrical_period_us_sp+periodo_var) )
	{
		//La velocidad es menor que el rango permitido
		return SPEED_LOWER;
	}
	else if ( periodo_avg < (gv.motor_electrical_period_us_sp-periodo_var) )
	{
		//La velocidad es mayor que el rango permitido
		return SPEED_OVER;
	}
	else
	{
...
void motor_3phase_speed_change (int32_t modif)
{
	...
	if(modif == SPEED_LOWER)
	{
		if(actual_pwm_set < MOTOR_MAX_SPEED_PWM)
		{
			gv.pwm_duty_set_point += SET_POINT_PWM_DUTY_INC_DEC;
		}
		else
		{
