...
int8_t ui_speed_value = 1;
int8_t ui_heater_value = 0;
...
void ui_update (void)
{
    ...
    if (switches_state & (1<<UI_SWITCH_COLDSHOT_SHIFT_MASK))
    {
		...
		{
			aux=0;
			ui_led_slide(UI_HEATER, UI_DOWN);
			ui_led_slide(UI_HEATER, UI_DOWN);
			ui_led_slide(UI_HEATER, UI_DOWN);
		}
    ...
    if ( (switches_state & (1<<UI_SWITCH_INC_FAN_SHIFT_MASK)) != 0)
    {
		...
		if (ui_speed_value <= UI_SPEED_2)
		{
			ui_speed_value++;
			ui_led_force(UI_FAN,ui_speed_value);
		}
    }
    ...
    if ( (switches_state & (1<<UI_SWITCH_INC_HEAT_SHIFT_MASK)) != 0)
    {
		...
		//ui_led_slide(UI_HEATER, UI_UP);
		if (ui_heater_value <= UI_HEATER_2)
		{
			ui_heater_value++;
			ui_led_force(UI_HEATER,ui_heater_value);
		}
    ...
    if ( (switches_state & (1<<UI_SWITCH_DEC_HEAT_SHIFT_MASK)) != 0)
    {
		...
		//ui_led_slide(UI_HEATER, UI_DOWN);
		if (ui_heater_value >= UI_HEATER_1)
		{
			ui_heater_value--;
			ui_led_force(UI_HEATER,ui_heater_value);
		}
